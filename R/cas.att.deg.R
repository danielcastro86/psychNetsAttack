cas.att.deg <- function(g){
  ...
  for(i in 1:(n-1)){
    vertex[i] <- V(g2)$names[which.max(igraph::degree(g2))] #cálculo e seleção do vértice a remover + registo do nome do vértice
    strength[i] <- max(igraph::degree(g2)) #registo do valor máximo da força
