plot.att <- function(attackscores){
    ...
    AvePathPlot <- ggplot(graphdata, aes(x=`Proportion of Nodes in the Network`,
      ...
      scale_y_continuous() +
      scale_x_continuous(breaks = c(rev(seq(from = 0, to = max(graphdata$`Proportion of Nodes in the Network`, na.rm = T)))),
    ...
    DensityPlot <- ggplot(graphdata, aes(x=`Proportion of Nodes in the Network`,
      ...
      geom_point(shape = 19, size = 2) +
      scale_y_continuous() +
      scale_x_continuous(breaks = c(rev(seq(from = 0, to = max(graphdata$`Proportion of Nodes in the Network`, na.rm = T)))), limits = c(0, max(graphdata$`Proportion of Nodes in the Network`, na.rm = T))) + theme_gray()
