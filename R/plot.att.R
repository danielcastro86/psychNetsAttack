plot.att <- function(attackscores){
    ...
    DensityPlot <- ggplot(graphdata, aes(x=`Proportion of Nodes Removed`,
      ...
      geom_point(shape = 19, size = 2) +
      scale_y_continuous() +
      scale_x_continuous(labels = function(x) paste0(x * 100, '%'), breaks = c(0,0.05,0.1,0.15,0.2,0.25,0.3,0.35,0.4,0.45,0.5,0.55,0.6,0.65,0.7,0.75,0.8,0.85,0.9,0.95,1)), limits = c(0, max(graphdata$`Proportion of Nodes Removed`, na.rm = T))) + theme_gray()
