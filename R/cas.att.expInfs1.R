cas.att.expInfs1 <- function(g){
  ...
  animation::saveGIF({ for(i in 1:(n-1)){
    vertex[i] <- V(g2)$names[which.max(networktools::expectedInf(g)$step1)] #cálculo e seleção do vértice a remover + registo do nome do vértice
    strength[i] <- max(networktools::expectedInf(g)$step1) #registo do valor máximo da força
